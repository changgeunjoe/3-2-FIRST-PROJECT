...
void CMesh::Render(ID3D12GraphicsCommandList *pd3dCommandList, int nSubSet)
{
	...
	pd3dCommandList->IASetVertexBuffers(m_nSlot, 1, &m_d3dPositionBufferView);//¹öÅØ½º ºä¿Í Æ÷Áö¼Ç ºä ¹«½¼Â÷ÀÌÁö
...
//	}
//
//	m_pd3dVertexBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf3Positions, sizeof(XMFLOAT3) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dPositionUploadBuffer);
//
...
void CStandardMesh::LoadMeshFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, FILE *pInFile)
{
	...
	for ( ; ; )
	{
		...
		else if (!strcmp(pstrToken, "<SubMeshes>:"))
		{
			...
			if (m_nSubMeshes > 0)
			{
				m_pnSubSetIndices = new UINT[m_nSubMeshes];
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext):CMesh( pd3dDevice,  pd3dCommandList)
{
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			pVertices[i].m_xmf2TexCoord = XMFLOAT2(float(x) / float(cxHeightMap - 1), float(czHeightMap - 1 - z) / float(czHeightMap - 1));//ÅØ½ºÃÄ ÁÂÇ¥
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float Height,int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext) :CMesh(pd3dDevice, pd3dCommandList)
{
	m_nVertices = nWidth * nLength;
	//	m_nStride = sizeof(CTexturedVertex);
	m_nStride = sizeof(CDiffusedTexturedVertex);
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		...
		{
			fHeight = OnGetHeight(x, z, pContext);
			pVertices[i].m_xmf3Position = XMFLOAT3((x * m_xmf3Scale.x), Height, (z * m_xmf3Scale.z));
			pVertices[i].m_xmf4Diffuse = Vector4::Add(OnGetColor(x, z, pContext), xmf4Color);
			pVertices[i].m_xmf2TexCoord = XMFLOAT2(float(x) / float(cxHeightMap - 1), float(czHeightMap - 1 - z) / float(czHeightMap - 1));//ÅØ½ºÃÄ ÁÂÇ¥
			if (fHeight < fMinHeight) fMinHeight = fHeight;
			if (fHeight > fMaxHeight) fMaxHeight = fHeight;
...
CRawFormatImage::~CRawFormatImage(void)
{
...
CSceneMesh::CSceneMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, char* pstrFileName) :CMesh(pd3dDevice, pd3dCommandList)
{
...
void CSceneMesh::LoadMeshFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, char* pstrFileName)
{
...
void CSceneMesh::Render(ID3D12GraphicsCommandList* pd3dCommandList, int nSubset)
{
	pd3dCommandList->IASetPrimitiveTopology(m_d3dPrimitiveTopology);
