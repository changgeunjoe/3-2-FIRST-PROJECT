...
{
	if (nPipelineState == 0)
		return(CShader::CompileShaderFromFile(L"GeometryShader.hlsl", "VSParticleStreamOutput", "vs_5_1", &m_pd3dVertexShaderBlob));
	else
		return(CShader::CompileShaderFromFile(L"GeometryShader.hlsl", "VSParticleDraw", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CParticleShader::CreateGeometryShader(int nPipelineState)
{
	if (nPipelineState == 0)
		return(CShader::CompileShaderFromFile(L"GeometryShader.hlsl", "GSParticleStreamOutput", "gs_5_1", &m_pd3dGeometeryShaderBlob));
	else
		return(CShader::CompileShaderFromFile(L"GeometryShader.hlsl", "GSParticleDraw", "gs_5_1", &m_pd3dGeometeryShaderBlob));
}
...
D3D12_SHADER_BYTECODE CParticleShader::CreatePixelShader(int nPipelineState)
{
	if (nPipelineState == 0)
		...
		return(CShader::CompileShaderFromFile(L"GeometryShader.hlsl", "PSParticleDraw", "ps_5_1", &m_pd3dPixelShaderBlob));
}
